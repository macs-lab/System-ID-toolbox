function varargout = foloe(y,u,na,nb,d,A0,Fin,lam1,lam0)
% FOLOE is used to identify a discrete time model of a plant operating in
% open-loop based on the output error method with filtered observation.
%
% [B,A]=foloe(y,u,na,nb,d,A0,Fin,lam1,lam0)
%
...
%
% A0 is the denominator of the initial model (fixed compensator)
%
...
%
% See also AFOLOE, OLOE, XOLOE, RLS and RELS.
%
% written by: A. Karimi, I.D. Landau
% 7th june 2002
%
% Modified by Xu Chen mail.xchen@gmail.com on 2011-09-05
...
if max(abs(roots(A0))) >= 1,
    error('A0 should be stable!');
end

na0=length(A0)-1;
np=max([na+1,nb+d,na0+1]);
...
for t=1:nd
    ...
    yuf=[phi(1) phi(np+1)]-[A0(2:na0+1) zeros(1,np-na0-1)]*[phi_f(1:np-1) phi_f(np+1:2*np-1)];
