function varargout=xoloe(y,u,na,nb,nc,d,Fin,lam1,lam0)
% XOLOE is used to identify a discrete time model of a plant operating in
% open-loop based on the extended output error method.
% 
% [B,A,C]=xoloe(y,u,na,nb,nc,d,Fin,lam1,lam0)
% 
...
% nc is the order of the numerator of the noise model (equal to or greater than na).
% 
...
if (nc<na), error('nc should be equal to or greater than na!'),end
...
for t=1:nd
    ...
    theta_vec(:,t) = theta(:);
        ...
        phi(i+1)=phi(i);phi(1)=-yhat;phi(np+1)=u(t);phi(2*np+1)=e_apost;
end
A=[1;theta(1:na)]';
B=[zeros(d+1,1);theta(na+1:na+nb)]';
C=[1;theta(na+nb+1:nth)+[theta(1:na);zeros(nc-na,1)]]';
